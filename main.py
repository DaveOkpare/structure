...
def main():
    ...
    fields = typer.prompt("Enter the fields you want to organize the extracted data into (comma-separated)")
    fields_list = [field.strip() for field in fields.split(',')]
    print(f"Fields to organize data: [bold green]{fields_list}[/bold green]\n")

    save_path = typer.prompt("Where would you like to save the extracted data? (Please provide the file path):")
